...
OS_STK StkTaskt01[STACKSIZEMGUI];
OS_STK StkTaskt02[STACKSIZEMGUI];
OS_STK StkTaskt03[STACKSIZEMGUI];
OS_STK StkTaskt04[STACKSIZEMGUI];
OS_STK StkTaskt05[STACKSIZEMGUI];
OS_STK StkTaskt06[STACKSIZEMGUI];
...
//-----------------------------------------------------------------------------
void RestoreScreen(MGUI_SAVESCR pEnderSave, unsigned short xi, unsigned short yi, unsigned short pwidth, unsigned short pheight) {
    unsigned short xf, yf;
    unsigned int ix, iy;
    unsigned int offset, posX, posY, modY, saveOffSet, saveOffSetAnt;
    unsigned char pixel;
...
{
    unsigned short cc = 0;
    unsigned char cchar, *vvarptr, vdisp = 0, vtec;
...
unsigned char message(char* bstr, unsigned char bbutton, unsigned short btime)
{
    ...
    RestoreScreen(vsavescr, xi,yi,pwidth + 5,pheight + 5);
...
void menuTask(void *pData)
{
    ...
    RestoreScreen(endSaveMenu, mx, my, 128, 44);
...
void importFile(void)
{
    ...
    RestoreScreen(vsavescr, 10,40,240,60);
    ...
    if (vwb == BTOK)
    {
        ...
        if (vresp == BTYES)
        {
            ...
            RestoreScreen(vsavescr, 10,40,240,70);
        }
    }
