...
unsigned char inputLineBasic(unsigned int pQtdInput, unsigned char pTipo)
{
    ...
    int editBaseX = 0, editBaseY = 0;

#define EDIT_SET_CURSOR_FROM_POS() \
    do { \
        int _abs = editBaseX + iPos; \
        int _row = editBaseY + (_abs / VDP_COLS); \
        int _col = _abs % VDP_COLS; \
        if (_row < 0) _row = 0; \
        if (_row >= VDP_ROWS) _row = VDP_ROWS - 1; \
        if (_col < 0) _col = 0; \
        if (_col >= VDP_COLS) _col = VDP_COLS - 1; \
        videoCursorPosRowY = _row; \
        videoCursorPosColX = _col; \
        vdp_set_cursor(videoCursorPosColX, videoCursorPosRowY); \
    } while (0)

#define EDIT_RESTORE_CHAR_UNDER_CURSOR() \
    do { \
        int _len = strlen(vbufInput); \
        if (iPos < 0) \
            iPos = 0; \
        if (iPos > _len) \
            iPos = _len; \
...
#define EDIT_PRINT_CHAR_SAFE(_ch, _move) \
    do { \
        if ((_ch) == 0x00) \
            printChar(0x20, (_move)); \
        else \
            printChar((_ch), (_move)); \
    } while (0)
    ...
    if (pTipo == 'S' || pTipo == 'I' || pTipo == 'R')
    {
        ...
        pIns = 0xFF;
        editBaseX = 0;
        editBaseY = videoCursorPosRowY;

        EDIT_SET_CURSOR_FROM_POS();
    }
        ...
        if (pTipo != '@')
        {
            switch (countCursor)
            {
                case 6000:
                    hideCursor();
                    if (pEdit)
                        EDIT_RESTORE_CHAR_UNDER_CURSOR();
                    break;
                case 12000:
                    showCursor();
                    countCursor = 0;
                    ...
                    else
                    {
                        ...
                        if ((iw + 1) <= pQtdInput)
                        {
                            ...
                            for (ix = iPos; ix <= iw ; ix++)
                            {
                                ...
                                EDIT_PRINT_CHAR_SAFE(vbufInput[ix + 1],1);
                            }
            ...
            else if ((vtec == 0x08 || vtec == 0x7F) && pEdit)  // Backspace
            {
                ...
                if ((vtec == 0x08 && iPos > 0) || vtec == 0x7F)
                {
                    ...
                    for (ix = iPos; ix < iw ; ix++)
                    {
                        ...
                        EDIT_PRINT_CHAR_SAFE(vbufInput[ix],1);
                    }
...
#undef EDIT_SET_CURSOR_FROM_POS
#undef EDIT_RESTORE_CHAR_UNDER_CURSOR
#undef EDIT_PRINT_CHAR_SAFE
}
...
{
    static unsigned char vTempPool[16][250];
...
char createVariableArray(unsigned char* pVariable, char pType, unsigned int pNumDim, unsigned int *pDim)
{
    ...
    char vLenVar = 0;
    int ix, vTam;
    long vAreaFree = vMemTotalArrayVar - ((unsigned long)(*nextAddrArrayVar) - (unsigned long)pStartArrayVar);
    long vSizeTotal = 0;
    unsigned char sqtdtam[20];
...
static void vdpEditSetCursorFromLogicalPos(int startX, int startY, int lineLen, int cursorPos)
{
    ...
    if (lineLen < 0)
        lineLen = 0;
    if (lineLen >= VDP_MAX_LINE)
        lineLen = VDP_MAX_LINE - 1;
    ...
    else
    {
        ...
        vdpEditCurY = startY + 1;

        while (localPos >= VDP_COLS && vdpEditCurY < (VDP_ROWS - 1))
        {
            localPos -= VDP_COLS;
            vdpEditCurY++;
        }
        ...
        if (localPos < 0)
            localPos = 0;
        if (localPos >= VDP_COLS)
            localPos = VDP_COLS - 1;

        vdpEditCurX = localPos;
    }
...
static void vdpEditRenderLogicalLine(int startX, int startY, int oldEndY, char *line, int cursorPos)
{
    ...
    lenLine = 0;
    while (lenLine < (VDP_MAX_LINE - 1) && line[lenLine] != '\0')
        lenLine++;
    rowsUsed = 1;
    firstWidth = VDP_COLS - startX;
    if (firstWidth <= 0)
    ...
    {
        remaining = lenLine - firstWidth;
        while (remaining > 0 && rowsUsed < VDP_ROWS)
        {
            rowsUsed++;
            remaining -= VDP_COLS;
        }
